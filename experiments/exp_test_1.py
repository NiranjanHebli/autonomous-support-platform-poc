...
def main():
    console.rule("[bold]Test: First 1 Question[/bold]")

    rows = []
    with open(GOLDEN_SET_PATH, newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))

    # Take only the first 1 row
    rows = rows[:1]
    ...
    # Reduced workers, increased timeout and retries for local Ollama
    ...
    console.print("\n[bold cyan]Test Results (1 question):[/bold cyan]")
    for metric in [
        "faithfulness",
        "answer_relevancy",
        "context_precision",
