...
ROOM_ID = os.getenv("MATRIX_ROOM_ID", "!oDXwgLUCnHlmqpBgQc:matrix.org")
...
def get_event_sender(client: httpx.Client, room_id: str, event_id: str) -> str:
    ...
    try:
        ...
        print(f"Warning: Could not fetch original event {event_id} to find recipient: {e}")
        return ""
...
def ingest_messages(access_token: str, room_id: str):
    ...
    print(f"Syncing messages since token: {since_token[:15] + '...' if since_token else 'BEGINNING'}")
    ...
    for event in events:
        if event.get("type") == "m.room.message":
            ...
            recipient_username = ""
            reply_event_id = content.get("m.relates_to", {}).get("m.in_reply_to", {}).get("event_id")
            if reply_event_id:
...
if __name__ == "__main__":
    # If running locally, you can use python-dotenv to load from .env
    # In Databricks, you might use dbutils.secrets.get() instead.
    try:
    ...
    if not token:
        print("ERROR: ACCESS_TOKEN not found in environment.")
        exit(1)
