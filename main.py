...
def get_client() -> MatrixClient:
    ...
    if not access_token:
        console.print("[bold red]Error:[/] ACCESS_TOKEN not found in .env file.")
...
@app.command()
def view(room_id: str, limit: int = typer.Option(20, help="Number of messages to view")):
    """View recent messages in a specific room (auto-joins if needed)."""
    client = get_client()

    with console.status(f"Joining and fetching messages for {room_id}...", spinner="dots"):
        try:
...
def reply(
    room_id: str, 
    message: str,
    reply_to: str = typer.Option(None, "--reply-to", "-r", help="Event ID of the message to reply to")
):
    ...
    with console.status(f"Sending message to {room_id}...", spinner="dots"):
        try:
            ...
            console.print(f"[bold green]Message sent successfully![/] Event ID: {event_id}")
        except Exception as e:
